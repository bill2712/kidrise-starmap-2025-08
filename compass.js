// compass.js (最終修正版 - 修正 datapath 拼寫錯誤)
document.addEventListener("DOMContentLoaded", function() {
    ...
    function init() {
        ...
        ui.analyzeButton.style.backgroundColor = '#666';
        if (ui.visibleStarsList) ui.visibleStarsList.innerHTML = `<p class="stars-placeholder">正在載入位置及星體數據...</p>`;
        
        const celestialConfig = {
            projection: "stereographic", width: 1, 
    ...
    function updateTime() { if (ui.currentTime) ui.currentTime.textContent = new Date().toLocaleTimeString('zh-HK'); }
    function getLocation() {
        ...
        navigator.geolocation.getCurrentPosition(pos => {
            ...
            if (ui.visibleStarsList) ui.visibleStarsList.innerHTML = `<p class="stars-placeholder">錯誤：無法獲取您的位置，請檢查瀏覽器權限。</p>`;
        });
    }
    ...
    function buildCelestialIndex() { state.celestialData = []; if (Celestial.constellations) { Celestial.constellations.forEach(c => state.celestialData.push({ name: c.name, type: "constellation", id: c.id })); } if (Celestial.data?.stars?.features) { Celestial.data.stars.features.forEach(f => { const nm = f.properties?.name; if (nm && f.properties.mag < 2.5) state.celestialData.push({ name: nm, type: "star" }); }); } ["Sun","Moon","Mercury","Venus","Mars","Jupiter","Saturn","Uranus","Neptune"].forEach(p => state.celestialData.push({ name: p, type: "planet" })); state.celestialDataReady = true; checkReadyState(); }
    function analyzeDirection() { if (!state.locationReady || !state.celestialDataReady) { alert("數據仍在載入中，請稍候。"); return; } const capturedAzimuth = state.azimuth; if (ui.visibleStarsList) ui.visibleStarsList.innerHTML = `<p class="stars-placeholder">正在分析 ${Math.round(capturedAzimuth)}° 方向的星空...</p>`; setTimeout(() => { updateVisibleStars(capturedAzimuth); }, 100); }
    function updateVisibleStars(azimuth) { Celestial.date(new Date()); const visibleObjects = []; const centerCoords = Celestial.azimuthalToEquatorial({az: azimuth, alt: 30}, state.location); state.celestialData.forEach(item => { const searchParams = { type: item.type, name: item.name }; if (item.type === 'constellation' && item.id) { searchParams.id = item.id; } const itemCoords = Celestial.search(searchParams); if (itemCoords) { const distance = Celestial.distance(centerCoords, itemCoords); const currentPos = Celestial.equatorialToAzimuthal(itemCoords, state.location); if (currentPos.alt > 0 && distance <= 45) { visibleObjects.push(item); } } }); if (ui.visibleStarsList) { if (visibleObjects.length > 0) { ui.visibleStarsList.innerHTML = visibleObjects.slice(0, 5).map(item => { const detail = celestialDetails[item.name] || `這是${item.name}，一個美麗的宇宙天體。`; const typeClass = `star-type-${item.type.toLowerCase()}`; const borderColor = { constellation: '#3498db', star: '#f1c40f', planet: '#c2b280' }[item.type] || '#ccc'; return `<div class="star-details-card" style="border-left-color: ${borderColor};"><div class="star-details-header"><span class="star-type ${typeClass}">${getTypeName(item.type)}</span><h4>${item.name}</h4></div><p>${detail}</p></div>`; }).join(''); } else { ui.visibleStarsList.innerHTML = `<p class="stars-placeholder">在 ${Math.round(azimuth)}° 方向的地平線以上，目前沒有找到顯著目標。</p>`; } } }
