// compass.js (v3.0 - 遵循「一次定位，按需分析」的最終穩定版)
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function orientationHandler(event) {
        ...
        let diff = alpha - state.lastRawAzimuth;
        if (Math.abs(diff) > 180) {
            if (diff > 0) { state.rotationCount--; } else { state.rotationCount++; }
        }
        
        const cumulativeRotation = alpha + state.rotationCount * 360;
        state.lastRawAzimuth = alpha;
        state.azimuth = alpha; // 只更新實時方向
        
        if (ui.compassNeedle) ui.compassNeedle.style.transform = `rotate(${cumulativeRotation}deg)`;
    ...
    function analyzeDirection() {
        ...
        const capturedAzimuth = state.azimuth; // 鎖定當前方向
        if (ui.visibleStarsList) ui.visibleStarsList.innerHTML = `<p class="stars-placeholder">正在分析 ${Math.round(capturedAzimuth)}° 方向的星空...</p>`;
    ...
    function updateVisibleStars(azimuth) {
        Celestial.date(new Date());
        const visibleObjects = [];
        const centerCoords = Celestial.azimuthalToEquatorial({az: azimuth, alt: 30}, state.location);
        ...
        if (ui.visibleStarsList) {
            if (visibleObjects.length > 0) {
                ui.visibleStarsList.innerHTML = visibleObjects.slice(0, 5).map(item => {
                    const detail = celestialDetails[item.name] || `這是${item.name}，一個美麗的宇宙天體。`;
                    const typeClass = `star-type-${item.type.toLowerCase()}`;
                    const borderColor = { constellation: '#3498db', star: '#f1c40f', planet: '#c2b280' }[item.type] || '#ccc';
                    return `<div class="star-details-card" style="border-left-color: ${borderColor};"><div class="star-details-header"><span class="star-type ${typeClass}">${getTypeName(item.type)}</span><h4>${item.name}</h4></div><p>${detail}</p></div>`;
                }).join('');
            } else {
                ui.visibleStarsList.innerHTML = `<p class="stars-placeholder">在 ${Math.round(azimuth)}° 方向的地平線以上，目前沒有找到顯著目標。</p>`;
            }
        }
    }
    ...
    function init() {
        ...
        const celestialConfig = {
            projection: "stereographic", width: 1, datapath: "/kidrise-starmar-2025-08/data/",
            ...
            callback: function(err) { 
                if (err) return; 
                buildCelestialIndex(); 
                // 將 getLocation 移到 callback 中，確保數據載入後才獲取位置
                getLocation();
            }
        };
