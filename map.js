// map.js (電腦版 - 修正 UI 元素對應)
document.addEventListener("DOMContentLoaded", function() {
    
    const desktopContainer = document.getElementById('starmap-container-desktop');
    if (!desktopContainer || window.getComputedStyle(desktopContainer).display === 'none') {
        return;
    }

    if (typeof Celestial === "undefined") {
        console.error("電腦版：核心星圖函式庫 Celestial 未能成功載入。");
        desktopContainer.innerHTML = "<h1>抱歉，星圖核心元件載入失敗</h1>";
        return;
    }

    const ui = {
        messageElement: document.getElementById('message-desktop'),
            ...
            if (typeof DeviceOrientationEvent.requestPermission === 'function') {
                DeviceOrientationEvent.requestPermission().then(permissionState => {
                    if (permissionState === 'granted') {
                        ...
                        state.isSkyviewActive = false; button.textContent = '🔭 開啟陀螺儀'; button.classList.remove('active');
                    }
                }).catch(console.error);
            } else {
